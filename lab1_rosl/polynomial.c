...
PolynomialErrors evaluatePolynomial(const Polynomial* poly, const void* xValues, void* result) {
    if (!poly || !xValues || !result) return POLYNOMIAL_NOT_DEFINED;
    ...
    for (size_t i = 0; i < poly->size; i++) {
        ...
        memcpy(temp, poly->coefficients[i], poly->typeInfo->size);

        const void* x_i = (const char*)xValues + i * poly->typeInfo->size;
        poly->typeInfo->multiply(x_i, temp);
        poly->typeInfo->add(result, temp, result);
